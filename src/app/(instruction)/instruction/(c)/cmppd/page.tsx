...
function PredicateTableEntry(imm8: number, predicate: string, title: string, ops: Ops): React.ReactElement {
    ...
    return (
        <tr>
            ...
            <td>{title}</td>
            <td>{Ops[ops[0] as keyof typeof Ops]}</td>
            <td>{Ops[ops[1] as keyof typeof Ops]}</td>
            <td>{Ops[ops[2] as keyof typeof Ops]}</td>
            <td>{Ops[ops[3] as keyof typeof Ops]}</td>
            <td>{Ops[ops[4] as keyof typeof Ops]}</td>
        </tr>
    );
}
...
const PageData: InstructionPageLayoutProps = {
    ...
    description: (
        <>
            ...
            <p>
                For the legacy SSE and VEX versions, the results will all be a <NoWrap>64-bit</NoWrap> <em>integer</em> (not a double-precision floating-point) of either all zeros or ones.
