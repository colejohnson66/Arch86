...
const Exceptions = {
    ...
    UD: {
        CpuidMissing: <>If any of the required CPUID feature flags are not set.</>,
        CpuidNotEnabled: <>If any of the required CPUID feature flags need to be enabled, but are currently disabled.</>,
    ...
    NonAlignedMemory: (bytes: number) => (
        <>If a memory operand is not aligned to a <NoWrap>{bytes}-byte</NoWrap> boundary.</>
    ),
