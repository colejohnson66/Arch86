...
// maps an instruction mnemonic to its actual page
...
// handles cases where simple `toUpperCase` wouldn't produce the right content
...
export default function Instruction(props: InstructionProps): React.ReactElement {
    ...
    // `as` casts needed to silence TS; if it's in `Object.keys`, it can be used as the key
    let uppercased: string;
    if (props.useAliasForTitleCaseMapping) {
        uppercased = Object.keys(TitleCaseMapping).includes(aliasedName)
            ? TitleCaseMapping[aliasedName as keyof typeof InstructionTitles] as string
            : aliasedName.toUpperCase();
    } else {
        uppercased = Object.keys(TitleCaseMapping).includes(name)
            ? TitleCaseMapping[name as keyof typeof InstructionTitles] as string
            : name.toUpperCase();
    }

    let title: React.ReactElement | undefined = undefined;
    if (!props.noTitle && Object.keys(InstructionTitles).includes(aliasedName)) {
        const jsxTitle = InstructionTitles[aliasedName as keyof typeof InstructionTitles];
